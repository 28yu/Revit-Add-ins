...
`FormworkSheetCreator.CreateSheet` は型枠数量集計シートに以下のレイアウトで配置する：

```
┌─────────────────────────────────────────────┐
│  [サマリ集計表 型枠数量集計_合計]               │  ← 左上
│                                              │
│  [ホスト集計表] [リンクA集計表] [リンクB集計表] │  ← 中段に横並び
│  [リンクC集計表] [リンクD集計表] ...           │  ← 幅超過時は折り返し
│                                              │
│                          [3D ビュー 型枠分析] │  ← 右下
└─────────────────────────────────────────────┘
```
...
**API シグネチャ**:
- 引数 `mainScheduleIds` は `IList<ElementId>`（旧: 単一 `ElementId`）
- ホスト集計表 (`Host` 起点) を先頭に、リンク集計表を続けて並べる
- `FormworkCalculatorCommand` 側で `allMainScheduleIds` リストにホスト → リンク順に蓄積し
  まとめて渡す
...
**折り返しロジック**:
- 配置後の右端 (`placedRight`) がシート右マージン (`rightX`) を超え、かつ既に行頭ではない場合、
  - 配置済みのインスタンスを `doc.Delete()` で削除
  - `rowTopY = rowMaxBottomY - gap` で次行に移動
  - 同じ集計表を改めて配置 (`PlaceScheduleAt` を再呼び出し)
- 行内の最下端 `rowMaxBottomY` を追跡し、次行の Y 計算と最終 3D ビュー配置の基準とする

### 集計表の列幅自動調整（2026-05-14 更新）

**問題**: 旧計算式 (`maxUnits * 2.0 + 7mm`) は Revit の実描画より狭く、
日本語の見出し（例: 「型枠面積」）や 2 行ある値が **改行されてしまう** ことがあった。

**改善後の計算式** (`SetMainScheduleColumnWidths` / サマリ集計表 `StyleColumnHeaders`):

```
widthMm = max(headerUnits, maxValueUnits) * 2.6 + 12.0
        ↓                                   ↓     ↓
        ヘッダー行と値行を別計測             文字幅  左右パディング
        max を採用（優先順位: 値 > 見出し）
```
...
**優先順位ロジック**:
- Body 行 0 = 列ヘッダー、Body 行 1+ = データ・グループ見出し・合計
- 見出しと値を **個別に計測** (旧: 全行 max を 1 つで取っていた) し、
  両者の `max` を `effectiveUnits` として使う
- 結果:
  - 値が見出しより長い → 値の幅で改行を防ぐ
  - 値が見出しより短い → 見出しの幅で見出し改行を防ぐ
...
**診断ログ**:
- `[Sched] col N header=X.X maxValue=Y.Y → width=Z.ZZZ ft (~NNNmm)` で各列の計測結果を出力
- 旧 `maxUnits=X` 単一値より詳細で、見出し起因か値起因かが判別できる

### 現在のステータス
- **機能実装**: 完了（接触検出 Phase 1+2、3D視覚化、集計表、Excel出力、視点継承、リンク集計表の横並びシート配置、列幅自動調整改善）
...
28Tools の将来的な有償化に向けた方針。重い保護（難読化・サーバー認証）は**動作不安定・ユーザー離れリスク**が高いため採用せず、**シンプルかつ低コストな仕組み**で段階的に移行する。

### 想定する脅威
- 古い無料版を永続的に使い続けられる
- 有償化前にダウンロードした DLL を同僚にコピーして配布される
- 建築業界の B2B ユーザーが対象のため、**逆コンパイル・クラックは現実的リスクとして考慮しない**

### Phase 1: バージョン有効期限の組み込み（実装済み・v2.1〜）

各バージョンのコードに有効期限をハードコード。期限切れ後はリボンが表示されず最新版への更新を促す。

- 実装場所: `Licensing/ExpiryManager.cs`
- 期限切れチェック: `Application.OnStartup()` の冒頭で `ExpiryManager.IsExpired` を確認
- 期限切れ前 30 日になると警告 TaskDialog を表示（`ExpiryManager.ShouldShowWarning`）
- 多言語対応: `Expiry.Title` / `Expiry.ExpiredMessage` / `Expiry.WarningTitle` / `Expiry.WarningMessage`
- **新バージョンリリース時は `ExpiryDate` を必ず1年後に更新すること**
- サーバー不要、オフライン環境対応、難読化不要 → **ゼロランタイムオーバーヘッド**
- PC日付変更で回避可能だが B2B 用途として許容

### Phase 2: ライセンスキー方式（有償化時に実装予定）

購入時にメールで `license.dat` を発行 → DLL と同じフォルダに配置 → 起動時にローカルで署名検証。

**設計方針**:
- ライセンスキーファイル: ユーザーID + 有効期限 + 許可機能フラグ + RSA/HMAC 署名
- サーバー常時稼働不要（購入処理時のみ稼働）
- オフライン環境でも動作（建設現場で安心）
- 難読化不要（キーがないと動かない設計）
- DLL コピーされても `license.dat` がないと動かない

**実装フロー（予定）**:
1. 購入サーバー（最小限）でキー発行・メール送信
2. DLL 起動時に `license.dat` を読み込み → RSA 公開鍵で署名検証
3. 期限切れ・無効キー時は機能制限 or 起動拒否

### Phase 3: 機能の有償化階層（有償化時に検討）

| プラン | 含まれる機能 | 価格イメージ |
|--------|------------|--------|
| 無料 | 通り芯・シート・ビュー操作・部屋タグ | 無料 |
| Standard | + 構造系（梁色分け・型枠・耐火被覆） | 年額 |
| Pro | + Excel連携・全機能 | 年額 |

段階的な有償化で既存ユーザーの反発を最小化。

### Phase 4: 既存無料版ユーザーの自然移行

Phase 1（有効期限）があれば自然解決:
- v2.1（無料版）: 2027/6/1 で期限切れ
- v3.0（有償版）: ライセンスキー必須、基本機能は引き続き無料
- 旧無料版ユーザーは期限切れと同時に v3.0 への移行が必要になる

### 重要：避けるべき対策（コスト・リスクが大きすぎる）

| 対策 | 不採用の理由 |
|------|------------|
| 難読化（ConfuserEx 等） | WPFバインディング破壊、リフレクション失敗、AV誤検知、デバッグ困難化 |
| オンライン認証サーバー | サーバー運用コスト永続、ネット必須化、社内 Proxy 経由で誤検知 |
| マシンロック | マシン構成変更で正規ユーザーが弾かれる、苦情対応コスト増 |
| ネイティブコード化 | 開発コスト膨大、Revit API は C# 前提 |
