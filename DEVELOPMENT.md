...
あなた: 開発指示 → Claude: コード編集 → push
  ...
  → AutoBuild.ps1 が変更を検知 → 自動 pull → ビルド → デプロイ
  → Windows 通知「ビルド完了」
  → あなた: Revitを再起動してテスト
```
...
リポジトリのルートディレクトリで PowerShell を開き、以下を実行するだけです：

```powershell
cd C:\path\to\Revit-Add-ins
.\AutoBuild.ps1
```

スクリプトは main ブランチのリモート更新を30秒ごとに監視します。変更を検知すると、自動で `git pull` → `QuickBuild.ps1`（ビルド & デプロイ）を実行し、Windows デスクトップ通知で結果を表示します。

### オプション

```powershell
.\AutoBuild.ps1 -Interval 60    # 60秒間隔で監視（デフォルト: 30秒）
```
