...
| **シート統合** | 全カテゴリを1枚のシート（「データ」／英語版は「Data」）にまとめて出力 |
...
> 💡 ヘッダーの「要素ID」「カテゴリ」やシート名、編集可否マーカー（`(*変更不可)` 等）は、**書き出し時のアドイン言語**で出力されます。読み込み側は3言語すべての表記を認識するため、**別の言語で書き出した Excel もそのまま取り込めます**（日本語固定だった旧バージョンの Excel も読めます）。
