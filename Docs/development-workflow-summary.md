# Tools28 開発ワークフローまとめ

## 登場人物の相関図

### 登場人物の紹介

| 登場人物 | ドラマでの役割 | 説明 | 誰が動かす？ |
|---|---|---|---|
| **あなた（開発者）** | 監督・プロデューサー | 何を作るか決めて、指示を出す人。コードは書かない | **あなた自身** |
| **Claude Code** | 脚本家 | AIがコード（プログラムの設計図）を全て書いてくれるツール | AI が自動で動く |
| **PowerShell** | 現場スタッフ | ビルド（コードを実行可能なファイルに変換すること）やデプロイ（完成したファイルを所定の場所に配置すること）の実作業を行う | **あなたが実行** |
| **GitHub** | 台本倉庫 + 配達係 | コードを保管・共有するサービス。リリース（完成版を公開すること）も自動で行う | 保管は手動 / リリースは自動 |
| **NuGet** | 小道具係 | API（他のソフトの機能を借りるための窓口）の部品を自動で調達してくる | 自動で動く |
| **Revit** | 舞台 | 開発したアドイン（追加機能）が実際に動く場所 | **あなたが操作** |

### 相関図

```
┌──────────────────────────────────────────────────────────────────┐
│  凡例:  🙂 あなたが行う   🤖 AIが行う   ⚙ 自動で動く          │
└──────────────────────────────────────────────────────────────────┘


                 【開発フェーズ】

                  ┌─────────────────────┐
                  │   🙂 あなた（監督）  │
                  │                     │
                  │ ・作りたい機能を考える│
                  │ ・仕様をまとめる     │
                  └──────┬──────────────┘
                         │
                         │ 指示を出す
                         v
                  ┌──────────────┐
                  │ 🤖 Claude    │
                  │    Code      │
                  │（AI脚本家）  │
                  │              │
                  │ コードを     │
                  │ 全て書く     │
                  └──────┬───────┘
                         │
                         v
              +─────────────────────+
              |                     |
              |  コード（設計図）   |
              |                     |
              +─────────────────────+
                    |          |
  🙂 保管する      |          |
  （git push）      |          |
                    v          v

                 【ビルド＆テストフェーズ】

              🙂 PowerShell を実行する
              （QuickBuild.ps1 を起動）
                         |
          +--------------+--------------+
          |                             |
          v                             v
   ⚙ ビルド（変換）する          ⚙ デプロイ（配置）する
          |                             |
          |                             v
          |                     🙂 Revit を起動して
          |                        動作を確認する
          |
          |  ⚙ 部品を自動で届ける
          |<-----------  NuGet
          |            （小道具係）
          |
          |
          |        NG（不具合あり）→ 🙂 Claude Code に修正を指示 → 最初に戻る
          |
          v  OK（動作確認完了）

                 【リリースフェーズ】

   🙂 タグ（バージョン番号の
      目印）を付けて送る
         |
         v
      GitHub  ──────>  ⚙ GitHub Actions（自動スタッフ）
    （台本倉庫）            |
                           |  ⚙ 全バージョンを自動で
                           |    ビルド（変換）＆ ZIP 作成
                           |
                           v
                      ⚙ GitHub Releases
                     （完成品の受け渡し窓口）
                           |
                           v
                     ユーザーがダウンロード
```

### あなた・AI・自動の役割まとめ

| やること | 誰が？ | 具体的な操作 |
|---|---|---|
| 作りたい機能を考える | 🙂 あなた | アドインの仕様（何をどう動かすか）をまとめる |
| Claude Code に指示を出す | 🙂 あなた | チャットで「こういう機能を作って」と伝える |
| コードを書く・修正する | 🤖 AI (Claude Code) | 指示に基づいてコードを全て自動で書く。不具合修正も Claude Code に任せる |
| ビルド（変換）を実行する | 🙂 あなた → ⚙ 自動 | `QuickBuild.ps1` を実行すると、あとは自動で変換される |
| デプロイ（配置）する | ⚙ 自動 | ビルド完了後、自動で Revit のフォルダにコピーされる |
| Revit で動作確認する | 🙂 あなた | Revit を起動して「28 Tools」タブから機能をテスト |
| GitHub に保管する | 🙂 あなた | `git push`（コードをサーバーに送る）を実行 |
| リリース（公開）する | 🙂 あなた → ⚙ 自動 | タグを付けて送ると、あとは自動でZIP作成＆公開 |
| API（機能の窓口）部品の取得 | ⚙ 自動 | NuGet がビルド時に裏で自動調達する |

### 関係のまとめ

- **あなた** → 作りたい機能を考えて → **Claude Code** に指示を出す
- **Claude Code**（AI）→ 指示に基づいてコードを全て書く（新機能も修正も全て）
- **あなた** → **PowerShell** でビルド（変換）を実行 → 自動でデプロイ（配置）される
- **あなた** → **Revit** で動作確認 → 問題があれば Claude Code に修正を指示
- **あなた** → **GitHub** にタグ push → 自動でビルド → ZIP 作成 → 公開

> **初心者向けメモ**: 基本的な流れは「あなたが考えて指示 → Claude Code が書く → 自動でビルド → あなたが確認」です。コードは全て Claude Code に任せて OK。NuGet やデプロイは裏で自動的に動くので、最初は意識しなくて大丈夫です。

---

## 開発環境

- **言語**: C# / .NET Framework 4.8 (Revit 2021-2024) / .NET 8 (Revit 2025-2026)
- **対応バージョン**: Revit 2021, 2022, 2023, 2024, 2025, 2026
- **必要ツール**:
  - Claude Code（AIがコードを全て書く）
  - .NET Framework 4.8 開発ツール / .NET 8.0 SDK
  - PowerShell 5.0+
  - Revit (テスト用、デフォルトは 2022)
- **Revit API**: NuGet パッケージ `Nice3point.Revit.Api` 経由で取得（ローカル Revit のインストール不要でビルド可能）

---

## 日常の開発サイクル

1. **Claude Code にコードを書かせる**
   - 作りたい機能の仕様を Claude Code に伝える
   - Claude Code が `Commands/機能名/` にコマンドクラスを作成し、`Application.cs` にリボン登録も行う
2. **ビルド & デプロイ**
   - `.\QuickBuild.ps1` を実行（Revit 2022 用にビルド → Addins フォルダへ自動デプロイ）
   - 別バージョンで開発する場合は `dev-config.json` を編集、または `.\QuickBuild.ps1 -RevitVersion 2024` で指定
3. **Revit で動作確認**
   - Revit を起動（または再起動）し、「28 Tools」タブで新機能をテスト
4. **問題があれば Claude Code に修正を指示して繰り返し**
   - 不具合の内容を Claude Code に伝えて修正してもらう
   - ログ出力先: `C:\temp\Tools28_debug.txt`

---

## 新機能追加の手順

1. `Commands/` 配下に新しいフォルダとコマンドクラスを作成
   - `[Transaction(TransactionMode.Manual)]` 属性を付与
   - 名前空間: `Tools28.Commands.機能名`
2. `Application.cs` にリボンボタンを追加
3. （任意）32x32 PNG アイコンを `Resources/Icons/` に配置し、`.csproj` に `<Resource>` タグを追加
4. `.\QuickBuild.ps1` でビルド & テスト
   - SDK-style csproj のため `.cs` ファイルは自動認識（`<Compile Include>` 不要）

---

## リリース手順

1. **全バージョンをビルド**
   - `.\BuildAll.ps1` を実行 → 6バージョン分の DLL が `bin\Release\Revit{VERSION}\` に出力
2. **配布パッケージを作成**
   - `.\CreatePackages.ps1 -Version "X.X"` を実行
   - 出力: `Dist\28Tools_Revit{VERSION}_vX.X.zip`（各バージョン用 ZIP）
3. **動作確認**
   - 必要に応じて複数バージョンの Revit で検証
4. **コミット & プッシュ**
   - `git add .` → `git commit -m "..."` → `git push`
5. **GitHub Releases で公開**
   - `git tag vX.X` → `git push --tags`
   - GitHub Actions が自動で全バージョンのビルド → ZIP 作成 → Releases にアップロード

---

## 配布パッケージの構成

- ZIP ファイル名: `28Tools_Revit{VERSION}_vX.X.zip`
- 内容:
  - `28Tools/Tools28.dll` — メイン DLL
  - `28Tools/Tools28.addin` — マニフェストファイル
  - `install.bat` — ワンクリックインストール（管理者実行）
  - `uninstall.bat` — アンインストール
  - `README.txt` — インストール手順
- インストール先: `C:\ProgramData\Autodesk\Revit\Addins\{VERSION}\`

---

## CI/CD (GitHub Actions)

- **自動リリース**: タグ push (`git tag vX.X` → `git push --tags`) で起動
  - 全6バージョン (2021-2026) を自動ビルド
  - 配布 ZIP を作成し GitHub Releases にアップロード
- **手動実行**: GitHub Actions 画面 → "Build and Release" → Run workflow → バージョン番号を入力

---

## ビルドスクリプト一覧

| スクリプト | 用途 |
|---|---|
| `QuickBuild.ps1` | 開発用: 単一バージョンのビルド & Addins フォルダへデプロイ |
| `BuildAll.ps1` | リリース用: 全6バージョン一括ビルド |
| `CreatePackages.ps1` | 配布 ZIP 作成 |
| `GenerateAddins.ps1` | `.addin` マニフェストファイル生成 |
| `Deploy-For-Testing.ps1` | テスト用手動デプロイ |

---

## 注意事項

- トランザクションは `TransactionMode.Manual` を使用すること
- WPF ダイアログは XAML + コードビハインドで構成
- 条件付きコンパイルシンボル: `REVIT2021` ~ `REVIT2026` (バージョン別分岐に使用)
- プロジェクトファイル (`Tools28.csproj`) は SDK-style で、ターゲットフレームワークはバージョンに応じて自動切替え
