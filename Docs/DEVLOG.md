...
#### AutoBackup: 同期中ダイアログを許可リスト方式へ（第2段・完了）
同期中の**すべての**ダイアログを `OverrideResult(Ok)` で自動処理していた。
競合・権限・保存先の警告といった、本来ユーザーが判断すべき確認まで勝手に承認する状態。

許可リストを作るには `DialogId` の実データが要るため、まず記録だけを実装して運用ログを待った。
その結果、**同期が正常に完了したときのログには `DialogId=` の行が1つも出ない**ことが判明した。
＝ 正常時にダイアログは出ない。自動 OK が働くのは例外時だけ、つまり
**ユーザーが判断すべき場面でだけ発動していた**ことになる。

→ 許可リスト `AutoDismissDialogIds`（初期は空）を導入し、**載っていないものは自動処理しない**。
`OverrideResult` を呼ばなければ Revit が通常どおりダイアログを表示し、ユーザーが判断する。
普段の自動同期は従来どおり止まらずに完了する。

**教訓: 「止まると困る」を理由にした無条件の自動応答は、まず本当に発生するのかをログで確かめる。**
発生しないなら、そもそも自動応答は不要で、リスクだけが残っている。

⚠ `HashSet<string>` を追加した際、このファイルには `using System.Collections.Generic;` が
無かった（`var` と Linq だけで書かれていたため）。ビルドできない環境で編集するときは
**新しく使う型の using が揃っているか必ず確認すること。**

### シート一括作成にリスト入力モードを追加（案A: 両方式を切替）
マニュアルが説明していた「番号と名前のリストから作る」を、既存の「枚数＋図面No」方式を
残したまま追加した。ラジオボタンで切り替え、選んだ方の入力欄だけを表示する。

- 解析は `SheetCreationDialog.ParseSheetList`（`internal static`）に分離。
  空行無視／タブ無しは番号のみ／3列目以降無視／入力内の重複は先勝ち
- Excel からの貼り付けは **追加実装不要**。`AcceptsReturn` + `AcceptsTab` の複数行 TextBox に
  Ctrl+V すればタブ区切りのまま入る
- 既存シート番号との重複はスキップし、**スキップした番号を結果ダイアログに列挙**する
  （Revit はシート番号の重複を許さず、そのまま設定すると例外になる）
- ダイアログは `SizeToContent="Height"` にして、モードによる高さ差を吸収した

⚠ **`Window` 派生クラスで `Visibility.Collapsed` を単純名で書くと CS0176。**
`Window` は `Visibility` というインスタンスプロパティを継承しているため、
同名の列挙型をインスタンス経由の静的メンバーアクセスと解釈される。
`System.Windows.Visibility.Collapsed` と完全修飾すること
（ParameterCleanupDialog で同じ罠を踏んでおり、今回も書いた直後に検出した）。

### Excel ヘッダーの多言語化（書き出しは現在の言語 / 読み込みは全言語）
`要素ID` `カテゴリ`、統合シート名 `データ`、編集可否マーカー `(*変更不可)` 等が日本語固定で、
EN/CN ではヘッダーが混在表示になっていた。単純に多言語化すると、
**別言語で書き出した Excel の読み戻しが壊れる**（列を特定できない／マーカーを除去できない）。

→ `ParameterKindHelper`（種別ラベル `【共有】`）が既に使っていた方式をそのまま適用した。

- **書き出し**: `Loc.S()` で現在のアドイン言語
- **読み込み**: 全言語の候補と照合（日本語も残すので過去の Excel も読める）

新設した `ExcelHeaderNames`（要素ID/カテゴリ/統合シート名）と、
`ParameterHeaderMarker`（マーカー3種）の両方でこの方式に統一した。

⚠ 候補一覧は `Strings{JP,EN,CN}.cs` の値と**必ず一致**させること。ズレると読み戻しが静かに壊れる。
ビルドできない環境では、翻訳とコード内候補の一致をスクリプトで機械的に照合すると安全。

#### ついでに直した: 英語 Revit で「タイプ」列が編集不可扱いになる
`ParameterHeaderMarker.MarkerFor` が `p.RawName == "タイプ"` で判定していた。
パラメータ名は **Revit の言語**に従うため、英語版では `Type` になり判定が外れ、
「タイプ」列に要素参照マーカーが付いて灰色（編集不可）で書き出されていた。
→ 言語に依存しない `BuiltInParameter.ELEM_TYPE_PARAM` の Id で判定する。

**教訓: Revit が返す名前（パラメータ名・カテゴリ名）を日本語literalと比較しない。**
組み込みパラメータ Id / BuiltInCategory で判定する。

### ボタンアイコンの青を Revit の既定アイコンに合わせた（2026-08-28）

Revit 2024 以降、既定アイコンの青が淡くなったため、アドインのアイコンもそれに揃えた。

- **目標色**: `#67B1E6`（H=205° S=55.2% V=90.2%）。ユーザーが Revit の画面から
  カラーピッカーで実測した値
- **変換前の主色**: `#0066CC`（H=210° S=100% V=80%）

#### 単純な置換ではなく HSV の関係で変換する
アイコンには陰影・グラデーション・アンチエイリアスがあり、青は1色ではない
（実測で `#0066CC` `#1E90FF` `#2288DD` `#2563EB` `#B3D9FF` `#CCE5FF` など多数）。
「`#0066CC` を `#67B1E6` に置換」では単色部分しか変わらず、陰影が取り残される。

→ アンカー `#0066CC → #67B1E6` の**関係**を求め、青系ピクセル全体に同じ関係を適用する。

```
H: -5.0°       (210° → 205°)
S: ×0.552      (100% → 55.2%)
V: 1-(1-V)×0.49 (80% → 90.2%)  ← 白へ寄せる曲線。明るい色でクランプしない
```

V を単純な倍率（×1.127）にすると、既に明るい `#CCE5FF` 等が 100% を超えて
クランプされ、階調が潰れる。**「白との距離を一定比率で詰める」曲線**にすると
アンカーを正確に通しつつ、どの明度でもクランプしない。

#### 変換対象の判定
色相 185〜260°、かつ**彩度 15% 以上**。彩度の下限が重要で、これにより
`formwork.png` の構造物グレー `#A2A7B0`（S=8%）のような「青っぽいグレー」を巻き込まない。

#### 除外したアイコン（`scripts/recolor-icons.py` の EXCLUDE）
- `flag_us_*.png` — **アメリカ国旗の紺色 `#3C3B6E`**。一括変換すると国旗が壊れる
- `flag_jp_*.png` / `flag_cn_*.png` — 国旗（青は含まないが明示的に除外）
- `room_3d_color.png` — 青/緑/橙/紫の**4色パレット**。青だけ変えると配色の意味が壊れる
- `formwork.png` — ブランド青ではなく構造物のグレー

#### ⚠️ この変換は冪等ではない
変換後のアイコンに再度かけると色がさらにずれる。`scripts/recolor-icons.py` は
`--from` / `--to` を引数で受け、docstring に適用履歴を残す形にした。
実行前に必ず `--dry-run` で確認すること。

#### 副作用: 白抜き文字のコントラスト低下
青が淡くなるため、**白抜き文字を載せているアイコン**は白との
コントラスト比が 5.5:1 → 2.3:1 に低下する（`manual_16` `ver_16` `view_template`）。
形だけのアイコン（漏斗・矩形など）は Revit 自体が淡いので問題にならない。

**教訓: 色を一括変換するときは、白抜き文字・多色パレット・国旗のように
「その色である意味」を持つ画像を先に洗い出して除外する。**
彩度の下限と除外リストの両方で守ると取りこぼしにくい。

#### 追補: 白抜き文字は同色相のやや濃い青にした
`manual_16` / `ver_16` / `view_template` / `view_template_16` は白抜きの
「?」「i」「横線」を載せているため、`#67B1E6` では 16px 表示で読めなくなる。
→ **色相 205° は Revit と揃えたまま**、彩度・明度だけ戻した `#1D7CBF` を使う。
コントラスト比は 2.2:1 → 3.7〜4.5:1 に回復し、変換前より淡いので Revit の雰囲気は保てる。

B から C へ再変換すると丸め誤差が乗るため、**一度 git で変換前に戻してから
別のアンカーで変換し直す**こと。そのために `--only` オプションを追加した。

#### 結論: 一括適用はやめ、3機能のみに留めた
実機のリボンで確認した結果、**元の濃い青のほうが良いアイコンが多かった**ため、
一括適用は取りやめ、以下の3機能だけ Revit の青にした。他は変換前へ戻した
（戻したファイルは変換前とバイト単位で一致することを検証済み）。

| 機能 | アイコン | 適用色 |
|--|--|--|
| フィルタ整理 | `filter_management` | `#67B1E6` |
| DWG表示移行 | `dwg_layer_transfer` | `#67B1E6` |
| テンプレート整理 | `view_template` | `#1D7CBF`（白抜きの横線があるため濃いめ） |

**教訓: 見た目の一括変更は、まず少数に適用して実機で確認してから広げる。**
コントラスト比のような数値だけでは「元のほうが良い」という判断は拾えない。
今回は変換スクリプトに `--only` があったため、範囲を絞り直すのが容易だった。

#### 追補: 凡例を描いたアイコンも除外した
`beam_top_level` / `beam_under_level` は青 `#1E90FF` / 黄 `#DAB92F` / 桃 `#FF8094` の
**3色凡例**を持つ（実測でピクセル数まで同一）。青だけ淡くすると凡例の統一感が崩れるため除外。

⚠️ `fire_protection.png` も**まったく同じ3色凡例**を持つ（同じ色・同じピクセル数）。
今回は指示範囲外のため変換したままにしてあるが、同じ理由で除外する余地がある。

### 未対応で残した項目（次回以降）
- **梁色分けのフィルタ削除**: `doc.Delete(filterId)` はプロジェクト全体からフィルタを消すため、
  他ビューに手動適用していた場合もそこから消える。対象はツールが作った `梁天端_` 系のみで
  「上書き」チェックが必要なため、現状維持と判断。
- **AutoBackup の同期中ダイアログの許可リスト**: 許可リスト方式へ切り替え済み。
  実運用で `DialogId` が記録されたら、安全と判断できるものだけを
  `AutoDismissDialogIds` に追加していく。
...
## DwgLayerTransfer: UIを左右2ペイン（移行元／移行先）の段階選択方式に変更（2026-08-05）

### 背景
初版は「全ビュー × 全DWG を一括で対応付ける表」だったが、ユーザー要望により
**左＝移行元モデルのデータ情報 / 右＝移行先モデルのデータ情報**の2ペイン構成へ変更。
左でビュー→DWGと絞り込んで参照元を1つ決め、右で反映先のビュー（複数可）とDWGを選ぶ。

上部（移行元モデル・移行先モデル・移行の単位）は従来どおり。

### 「そのビューにあるDWG」の判定
左でビューを選ぶと、そのビューにあるDWGだけが下のリストに出る。判定は `ImportInstance.OwnerViewId`:

| OwnerViewId | 意味 | 扱い |
|--|--|--|
| 無効 (`InvalidElementId`) | モデルに読み込み／リンクした DWG | 全ビューに現れる |
| 有効 | そのビューに貼り付けた DWG（ビュー固有） | 当該ビューのみ |

`DwgDefinition` に `AppearsEverywhere` / `OwnerViewIds` を持たせ、`AppearsInView(viewId)` で判定。
**ImportInstance が1つも無い DWG カテゴリは `AppearsEverywhere = true` にする**
（インスタンスを消してもカテゴリは残るため、取りこぼすと設定を移せなくなる）。
ビューテンプレートはジオメトリを持たず全DWGのカテゴリを制御するので絞り込まない。

`FilteredElementCollector(doc, viewId)` で可視要素を採る手もあるが、
**非表示にしたDWGが出てこなくなり「表示に戻す」用途で使えない**ため採用しなかった。

### 副次的な効果: 走査コストが激減
初版は全ビュー×全DWG×全レイヤの `GetCategoryOverrides` を起動時に回していた
（50テンプレート×500レイヤ＝25,000回）。新方式は**選択中の1ビュー×そのDWGだけ**を読むため、
ダイアログが即座に開く。`ReadSettings` のシグネチャも
`(doc, viewId, dwg) -> Dictionary<レイヤ名, 設定>` に単純化した。

`DwgLayerApplier.Apply` も「1DWG分のレイヤ設定 → 複数ビュー × 1DWG」になり、
DWG名の対応表・スナップショットの入れ子辞書が不要になった。

### ⚠️ XAML コメントに `--` を含めるとビルドが落ちる
`<!-- ---------- 左: ... ---------- -->` のような区切り線コメントは **XML として不正**
（コメント内に `--` を書けない）。XAML コンパイラがエラーにする。区切り線を入れたい場合は
`=` や `*` を使うか、通常のコメントにすること。

### その他
- 左右のビュー一覧はそれぞれ検索ボックス付き（`ICollectionView.Filter`）。
  「全選択／選択解除」は**検索で表示中の行だけ**を対象にする。
- 左でビューを選ぶと同名の移行先ビューを自動チェック、DWGを選ぶと同名の移行先DWGを自動選択。
...
## DwgLayerTransfer: 「設定が移行しない」調査メモ（2026-08-05）

診断ログ（`C:\temp\Tools28_debug.txt`）で切り分けた結果の記録。

### ログで確定した事実

移行先ビューがビューテンプレートに制御されている場合、ビューへの
`SetCategoryOverrides` は **例外** `Category cannot be overridden.` になる
（黙って無視されるのではない）。テンプレートへ振り替えれば書き込めた。

同じ操作を2回実行したときの、ビュー側の読み戻し値の変化:

```
1回目 実際(線色=-)              ← 適用前
2回目 実際(線色=128,128,128)     ← 1回目の書き込みがビューへ届いている
      伝播=27/27
```

**テンプレート経由でビューへ伝播している**ことは API で確認済み。
それでも利用者からは「移行していない」と見える → 検証項目の不足が原因。

### 検証の盲点（修正済み）

| 項目 | 問題 | 対処 |
|--|--|--|
| 表示/非表示 | `CanCategoryBeHidden` が false のとき書き込みも検証も丸ごと省略しており、非表示を再現できなくても「反映」と数えていた | 事前判定をやめて実際に書き、書けなかった件数を集計して結果に出す。`IsEffective` も実状態を必ず比較する |
| 線種 | `IsEffective` の比較対象から外していたため、線種が入らなくても成功扱い | 移行先で解決できた線種は照合する（未解決分は従来どおり別途報告） |

### 教訓

**「書けたか」ではなく「ユーザーが見る場所で効いたか」で検証する。**
振り替え先（テンプレート）を読み戻しても、そこに書けたことしか分からない。
検証は必ず元のビューに対して行う。

また、検証の比較項目に漏れがあると **成功と誤報告する**。
比較しない項目を作るなら、その旨をログに残して見えるようにしておくこと。

### 続報: 原因はオブジェクトスタイルだった（同日）

V/G を全項目そろえ、移行元ビューと移行先ビューを項目ごと突き合わせても
「全項目一致」なのに表示が変わらなかった。ここで前提を疑い直して判明した。

**DWG レイヤの見た目は 2 段構え。**

| | 場所 | 範囲 |
|--|--|--|
| 基準値 | オブジェクトスタイル ＞ 読み込みオブジェクト | モデル全体 |
| 上書き | ビューの V/G ＞ 読み込みカテゴリ | そのビュー（基準値への差分） |

V/G は上書きした項目しか持たない。実データでは 48 レイヤ中 V/G を持つのは 27 件で、
残り 21 件は完全に基準値依存だった。27 件も全項目を上書きしているわけではない
（線色 19 / 線幅 18 / 線種 19）。よって **V/G だけ移しても表示は揃わない。**

`DwgObjectStyle` を追加し、線の色・投影/切断の線の太さ・線種を移すようにした。
モデル全体に効くため、チェックボックスで選択可能にしている（既定 ON）。

### 教訓

**「移す項目を勝手に間引かない」。**
サーフェスパターンと透明度を「DWG には効かない」と決めつけて除外していたのも同じ誤り。
除外した項目に設定が入っていると、読まない・書かない・比較しないため、
**全項目一致と報告しつつ何も変わらない**という最悪の症状になる。

**症状が消えないときは、検証の精度を上げる前に前提を疑う。**
このケースでは検証を 4 回強化したが、いずれも「V/G の中で」正しいかを見ていただけで、
V/G の外に原因がある可能性を潰せていなかった。

### 続報2: 原因は「書き込み先の取り違え」と「モデル全体への副作用」（同日）

オブジェクトスタイルも移行対象に加えたところ、**選択していないビューの CAD 線種が破線に変わった**。
利用者のスクリーンショットとログを突き合わせて、2 つの別問題が同時に起きていたと判明。

#### 1. モデル全体を書き換えていた

オブジェクトスタイルの移行を既定 ON にしていた。これは V/G と違い**モデル全体**に効くため、
選んだビュー以外の見た目まで変わる。48 レイヤすべてが書き換わっていた。

→ 既定 OFF に変更。有効時は確認ダイアログで影響範囲を警告する。

**モデル全体に効く操作を既定 ON にしない。** 影響範囲がビュー単位の機能に混ぜる場合はなおさら。

#### 2. 書き込み先が利用者に見えていなかった

| | ビュー | テンプレート |
|--|--|--|
| 実際の書き込み先 | 維持管理用_B2FL(B1FL) | T_平面図_エリアプラン |
| 利用者が見ていた先 | 維持管理用_平詳_B02 | 施工_平面_詳細図 |

ビュー名が似ており、割り当てテンプレートが別だった。「設定が入っていない」のは当然で、
そこには一度も書いていなかった。

状態列のテンプレート名は `TemplateControlsImportVg` の判定が真のときだけ出しており、
判定が外れると何も表示されなかった。

→ 割り当てテンプレート名は判定に関わらず常に表示。結果ダイアログにも実際の書き込み先名を出す。

**間接的な書き込み先へ振り替える機能は、振り替え先を実行前・実行後の両方で必ず見せる。**

#### 3. 線種名の大文字小文字

`2_Hidden2` と `2_HIDDEN2` は別の LinePatternElement で、破線ピッチが異なりうる。
名前解決は完全一致を優先し、大文字小文字無視の一致は完全一致が無い場合のみ使う。

### 結論（2026-08-05・動作確認完了）

「移行しても設定が変わらない」の原因は**単一ではなく、次の順で3つ重なっていた**。

1. **失敗を握り潰していた** — `SetCategoryOverrides` の例外を空 catch で捨て、
   表示/非表示を書けただけで成功と数えていたため、何も入っていなくても成功と報告していた
2. **移す項目を間引いていた** — サーフェスパターン・透明度・詳細レベルを除外し、
   さらにオブジェクトスタイル（モデル全体の基準値）を対象外にしていた
3. **書き込み先が利用者に見えていなかった** — 割り当てテンプレートへ振り替わるのに
   その名前を実行前に確認できず、意図と違うテンプレートへ書いても気付けなかった

#### 最終的な設計

| | 内容 |
|--|--|
| 読み取り | 常にビュー単位。テンプレート由来／ビュー個別のどちらでも実際に効いている値を取る |
| 書き込み | ビュー → 効かなければ主ビュー → ビューテンプレート の順に試す |
| 検証 | 常に**元のビュー**で読み戻して確認。伝播しなければ SubTransaction ごと破棄 |
| 対象 | V/G は `OverrideGraphicSettings` 全項目。オブジェクトスタイルは既定OFFの任意 |
| 提示 | 割り当てテンプレート名を一覧に常時表示。結果に実際の書き込み先名を表示 |

#### この調査から得た原則

- **「書けたか」ではなく「ユーザーが見る場所で効いたか」で検証する**
- **移す項目を勝手に間引かない。** 間引いた項目に設定が入っていると、読まない・書かない・
  比較しないため「全項目一致」と報告しつつ何も変わらないという最悪の症状になる
- **症状が消えないときは、検証の精度を上げる前に前提を疑う。**
...
### 実機で出た不具合と修正（2026-08-28 動作確認）

**症状**: ファミリ出力を実行すると「エラー: 無視できません／ファミリとプロジェクト間ではコピーできません」
のモーダルが出て、キャンセルすると **中身が空の .rfa が保存され、モデルにも何も現れない**。
しかも元要素は非表示になっているので、ビューが空になったように見える。

**原因は3つ重なっていた**

1. **`ElementTransformUtils.CopyElements` はプロジェクト↔ファミリ間では使えない。**
   材質をファミリ文書へ持ち込むのに使っていたが、Revit がこれを失敗として登録する。
2. **失敗はコミット時にモーダルで出る。** ユーザーがキャンセルすると
   **ファミリ文書のトランザクションごとロールバック**され、作成済みの
   `FreeFormElement` 22 個がすべて消える。
3. **`Transaction.Commit()` の戻り値を見ていなかった。**
   ロールバックされたのに成功扱いで `SaveAs` したため、空の .rfa が出来上がった。

**修正**

- 材質は `Material.Create(famDoc, 同じ名前)` で**ファミリ側に作り直す**。
  色・透明度・光沢・滑らかさ・材質クラスだけ写す。同名の材質は、ファミリを
  プロジェクトへロードした時点でプロジェクト側の既存材質と対応付けられる。
  テンプレートに同名がある場合は `Material.Create` が例外になるので、先に検索して再利用する。
- `FamilyFailurePreprocessor`（`IFailuresPreprocessor`）を付け、警告は削除、
  エラーは**文言を収集した上でロールバック**する。ユーザーに意味不明なモーダルを出さない。
- `Commit()` の戻り値を必ず確認し、`Committed` 以外なら収集したエラー文言を添えて失敗を返す。
- 保存前に `FilteredElementCollector(famDoc).OfClass(typeof(FreeFormElement))` で
  **形状が本当に入っているか数え直す**。
- プロジェクト側でも、**元要素を非表示にする前に**生成物が立体形状を持つか確認し、
  持たなければトランザクションごとロールバックする。
  「中身が空の一般モデルを作り、元要素だけ消えたビュー」を絶対に残さない。

**教訓**: Revit の失敗ダイアログは「ユーザーがキャンセルするとトランザクションが黙って
ロールバックされる」経路になる。**書き込み系のトランザクションでは `Commit()` の戻り値を
必ず確認し、成果物の実体も検証してから次の工程（保存・非表示・削除）へ進む。**
