...
- **対応は全バージョン（2021〜2026）共通**。配置先も保留先もバージョン別に分かれているため、
  `[build:2022,2024]` のような複数バージョンビルドでも互いに影響しない。
  起動中かどうかの判定もバージョン単位（プロセスの実行ファイルパスで判別）なので、
  例えば Revit 2024 を開いたまま 2022 をビルドすると **2022 はそのまま反映**され、
  2024 だけが「再起動で反映」になる。
